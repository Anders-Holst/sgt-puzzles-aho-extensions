/*
 * Implementation of the Kakuro puzzle 
 *
...
#undef SHOWDIFF    /* Define this to see the actual difficulty in the status bar (for evaluation purposes only) */
...
const static struct game_params kakuro_presets[] = {
  {5, 0, 0, 9, 3},
  {7, 0, 0, 9, 1},
  {7, 0, 0, 9, 2},
  {7, 0, 0, 9, 3},
...
static bool game_fetch_preset(int i, char **name, game_params **params)
{
    ...
    if (i < 0 || i >= lenof(kakuro_presets))
        return false;

    ret = snew(game_params);
    *ret = kakuro_presets[i]; /* structure copy */

    sprintf(buf, "%dx%d", ret->size, ret->size);
    if (ret->oddeven_mode)
      sprintf(buf+strlen(buf), ", odd/even mode");
    else if (ret->nosame_mode)
      sprintf(buf+strlen(buf), ", no same combo");
    if (ret->max != 9)
...
static char *encode_params(const game_params *params, bool full)
{
    ...
    sprintf(ret, "%d,%d%sD%d", params->size, params->max,
...
static config_item *game_configure(const game_params *params)
{
    ...
    ret[ind].u.choices.choicenames = ":Normal:Odd/even restriction:No same combinations";
    ret[ind].u.choices.selected = (params->oddeven_mode ? 1 : params->nosame_mode ? 2 : 0);

    ind++;
    ret[ind].name = "Difficulty";
    ret[ind].type = C_CHOICES;
    ret[ind].u.choices.choicenames = ":Trivial:Easy:Medium:Hard:Extreme";
...
static const char *validate_params(const game_params *params, bool full)
{
    if (params->nosame_mode) {
      if (params->size < 3 || params->size > 9)
        return "In no-same mode, grid size must be between 3 and 9";
    } else {
...
#ifdef MULTIDIGIT
    if (params->nosame_mode) {
      if (params->max < 7 || params->max > MAXNUM)
        return "In no-same mode, max slot value must be between 7 and " STR(MAXNUM);
    } else {
...
static Slot *new_Slot(KakuroBoard *kb, int ind)
{
    int i;
    Slot *sl = snew(Slot);
...
static KakuroBoard *new_KakuroBoard(const game_params* p)
{
    KakuroBoard *kb = snew(KakuroBoard);
    kb->par = p;
    kb->slots = 0;
    kb->runs = 0;
    kb->nslots = 0;
...
static void clean(KakuroBoard *kb)
{
    int i;
...
static void delete_KakuroBoard(KakuroBoard *kb)
{
    clean(kb);
    if (kb->candidate)
        sfree(kb->candidate);
...
static void reset(KakuroBoard *kb)
{
  int i;
  Run* r0;
...
static int check_correct_form(KakuroBoard* kb)
{
  int i, maxlen, ok;
...
static void setup_runs(KakuroBoard *kb)
{
    int i, j, k, cnt, cnt2;
    Run* r;
...
static void import_answer(KakuroBoard *kb, const char *str)
{
    int i, j;
    char ch;
    Slot *s;
...
static void export_answer(KakuroBoard *kb, char *str)
{
    int i;
    Slot *s;
    str[0] = 'S';
...
/*
static void export_clues(KakuroBoard *kb, char* str)
{
  int i, j;
  char* p;
  ...
  for (i=0; i<n; i++) {
    if (clues[i].v == -2)
      sprintf(p, "#");
    else if (clues[i].v != -1 && clues[i].h != -1)
      sprintf(p, "%d\\%d", clues[i].v, clues[i].h);
    else if (clues[i].v != -1)
      sprintf(p, "%d\\", clues[i].v);
    else if (clues[i].h != -1)
      sprintf(p, "\\%d", clues[i].h);
    else
      sprintf(p, "\\");
    p += strlen(p);
    if ((i+1)%sz)
...
static pair *get_clues(KakuroBoard *kb)
{
    int i, j;
    int sz = (kb->par->size+1);
    int n = sz*sz;
...
static void set_clues(KakuroBoard *kb, struct clues* cl)
{
  int i, j, k;
  int cnt=0, cnt2;
  Run* r;
...
/*
static void print_board(KakuroBoard* kb)
{
  int i, j;
  for (j=0; j<kb->par->size; j++) {
    for (i=0; i<kb->par->size; i++) {
      if (kb->slots[i+kb->par->size*j] == 0)
        printf(" #");
      else if (kb->slots[i+kb->par->size*j]->n == -1)
        printf(" *");
      else
        printf(" %d", kb->slots[i+kb->par->size*j]->n);
    }
    printf("\n");
  }
}
...
static void mutate_chain(KakuroBoard* kb, random_state *rs, Slot* sl)
{
...
static void randomize_squares(KakuroBoard *kb, random_state *rs, long *av)
{
...
static void mutate_squares(KakuroBoard* kb, random_state *rs, int m, long* av)
{
  int i, j, k, ns;
...
static int mutate_structure(KakuroBoard* kb, random_state *rs, int pos)
{
  int i, j, k, cnt;
  long mask, confl;
  Run** newruns;
...
static int contains_same(KakuroBoard *kb)
{
  int i;
  for (i=0; i<kb->nruns; i++)
...
static void eliminate_same(KakuroBoard *kb, random_state *rs)
{
...
static void maybe_split_run(KakuroBoard *kb, random_state *rs, int nsq, int pos, int dir)
{
  int tmp;
...
static void randomize_structure(KakuroBoard *kb, random_state *rs)
{
  int i, j, k, ncl, nsq;
  int n = kb->par->size*kb->par->size;
  while (1) {
...
static char* randomize_answer(KakuroBoard *kb, random_state *rs)
{
  int i, j;
  int n = kb->par->size*kb->par->size;
  clean(kb);
...
static char *mutate_answer(KakuroBoard *kb, random_state *rs, int m, long* av)
{
  int i, j, lim=3;
...
static Run *select_run(KakuroBoard *kb)
{
    int i, best = -1;
    for (i=0; i<kb->nruns; i++) {
        if (!kb->runs[i]->done &&
...
static long count_internal(KakuroBoard *kb)
{
  int numind, i;
    int *si;
    int *ii;
...
static void count_solutions(KakuroBoard *kb, const char *str, long limit, long *sol, long *it)
{
    int i, nsok = 1;
    float lp = 0.0;
    kb->iter = 0;
...
{
    KakuroBoard *kb = new_KakuroBoard(par);
    char *vec1, *vec2;
    long val1, val2;
    long iter1, iter2;
    float diff1, diff2;
...
#ifdef SHOWDIFF
    sprintf(global_buf, "Estimated difficulty: %.2f", hard);
#endif /* SHOWDIFF */
			...
			const char *aux, const char **error)
{
  if (aux)
    ...
    KakuroBoard *kb = new_KakuroBoard(state->par);
    long sol;
    kb->iter = 0;
    kb->quickret = 2;
    kb->itermax = 10000000;
...
static char* make_move_string(const game_state *state, game_ui *ui, int n)
{
  char buf[80];
...
{
  game_state *state = (game_state*)state0;
  ui->pending = 0;
  if (!ui->hcursor)
    ui->hshow = 0;
  midend_undo(state->clues->me); /* Remove the tentative pending state */
}
			    ...
			    int x, int y, int button)
{
...
#ifdef MULTIDIGIT
            if (ui->pending) {
              retstr = make_move_string(state, ui, ui->pending - '0');
	 ...
	 */
        ...
        if (ui->pending) {
          ...
          retstr = make_move_string(state, ui, n);
          abort_pending(state, ui);
        } else
#endif /* MULTIDIGIT */
        {
        ...
        for (x = 0; x < sz; x++)
          for (y = 0; y < sz; y++) {
            if (ret->grid[y*sz+x] == -1)
              ret->pencil[y*sz+x] = (((from->clues->oddeven == 2 ? 0 : 1) + x + y)%2 ? oddmask : evenmask);
        }
      } else {
...
static float *game_colours(frontend *fe, int *ncolours)
{
    ...
    ret[COL_EVENBG * 3 + 0] = 0.92F * ret[COL_BACKGROUND * 3 + 0];
    ret[COL_EVENBG * 3 + 1] = 0.92F * ret[COL_BACKGROUND * 3 + 1];
    ret[COL_EVENBG * 3 + 2] = 0.92F * ret[COL_BACKGROUND * 3 + 2];

    ret[COL_ODDBG * 3 + 0] = 1.07F * ret[COL_BACKGROUND * 3 + 0];
    ret[COL_ODDBG * 3 + 1] = 1.07F * ret[COL_BACKGROUND * 3 + 1];
    ret[COL_ODDBG * 3 + 2] = 1.07F * ret[COL_BACKGROUND * 3 + 2];
    if (ret[COL_ODDBG * 3 + 0] > 1.0) ret[COL_ODDBG * 3 + 0] = 1.0;
    if (ret[COL_ODDBG * 3 + 1] > 1.0) ret[COL_ODDBG * 3 + 1] = 1.0;
    if (ret[COL_ODDBG * 3 + 2] > 1.0) ret[COL_ODDBG * 3 + 2] = 1.0;
		      ...
		      struct clues *clues, int x, int y, long tile)
{
    ...
    if (par->oddeven_mode && clues->playable[y*sz+x] && !(tile & DF_HIGHLIGHT)) {
      int odd = ((clues->oddeven == 2 ? 0 : 1) + x + y)%2;
      draw_rect(dr, cx, cy, cw, ch,
                odd ? COL_ODDBG : COL_EVENBG);
    } else {
      draw_rect(dr, cx, cy, cw, ch,
    ...
    if (!clues->playable[y*sz+x]) {
        ...
        if (tile & DF_HAS_DIGIT_MASK) {
            ...
            draw_text(dr, tx + TILESIZE/2, ty + TILESIZE/2,
                      ...
                      (tile & DF_ERR_SLOT ? COL_ERROR : COL_USER), str);
